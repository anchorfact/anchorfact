...
function checkPythonSummary(report, summary) {
  ...
  if (!Number.isFinite(summary.health_source_ready_repair_queue_candidates ?? null)) {
    const message = valueLabel('health_source_ready_repair_queue_candidates', summary.health_source_ready_repair_queue_candidates, 'a number');
    failures.push(message);
    addFailure(report, message);
  }
