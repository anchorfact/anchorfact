...
atomic_facts:
  - id: "af-attention-mechanisms-deep-dive-1"
    statement: "Scaled dot-product attention computes Attention(Q,K,V) = softmax(QK^T/√d_k)V, where the scaling factor √d_k prevents dot products from growing too large, maintaining stable softmax gradients (Vaswani et al., 2017)."
    source_title: "Vaswani et al., NeurIPS (2017)"
    confidence: "high"
  - id: "af-attention-mechanisms-deep-dive-2"
    statement: "FlashAttention-3 (2024) achieves up to 740 TFLOPS on H100 GPUs by leveraging asynchronous processing, warp-group matrix multiply-accumulate, and interleaved block-wise computation — a 75% speedup over optimized standard attention."
