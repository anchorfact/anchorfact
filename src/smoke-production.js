...
export async function main() {
  ...
  assertOk(Array.isArray(artifactSummary.api_call_guidance?.minimum_valid_primary_calls) && artifactSummary.api_call_guidance.minimum_valid_primary_calls.some(call => call.path === '/api/context?q={query}&limit=3&format=markdown'), '/artifact-summary.json is missing minimum valid context call guidance', failures);
  assertOk(Array.isArray(artifactSummary.api_call_guidance?.minimum_valid_primary_calls) && artifactSummary.api_call_guidance.minimum_valid_primary_calls.some(call => call.path === '/api/evidence?q={query}&limit=3&format=markdown'), '/artifact-summary.json is missing minimum valid evidence call guidance', failures);
  assertOk(artifactSummary.api_call_guidance?.parameter_error_prevention?.do_not_call_bare_paths?.includes('/api/source'), '/artifact-summary.json does not warn against bare source API calls', failures);
  assertOk(artifactSummary.api_call_guidance?.parameter_error_prevention?.recovery_field_on_400 === 'machine_recovery', '/artifact-summary.json does not name machine recovery guidance field', failures);
