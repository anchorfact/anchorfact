...
test('buildContentHealthIndex publishes signed corpus health guidance', () => {
  const payload = buildContentHealthIndex({
    ...
    manifest: {
      ...
      articles: [
        ...
        {
          canonical_slug: 'ai/ai-for-fraud-prevention',
          title: 'AI for Payment Fraud Prevention',
          status: 'draft',
          is_draft: true,
          confidence_level: 'low',
          sources_verified: 0,
        ...
        {
          canonical_slug: 'ai/ai-warehouse-robotics',
          title: 'AI for Warehouse Robotics: Autonomous Forklifts',
          status: 'draft',
          is_draft: true,
          confidence_level: 'low',
          sources_verified: 0,
  ...
  assert(!payload.draft.repair_candidates.some(candidate => candidate.canonical_slug === 'ai/ai-for-fraud-prevention'), 'financial-risk AI drafts should stay out of automatic repair candidates');
  assert(payload.draft.strict_review_candidates.some(candidate => candidate.canonical_slug === 'ai/ai-for-fraud-prevention'), 'financial-risk AI drafts should be routed to strict review');
  ...
  assertEq(payload.draft.source_acquisition_candidate_count, 2);
  assertEq(payload.draft.source_acquisition_candidates[0].canonical_slug, 'ai/draft-a');
  assertEq(payload.draft.source_acquisition_candidates[1].canonical_slug, 'computer-science/api-draft');
  assertEq(payload.draft.repair_queue.candidate_count, 3);
  assertEq(payload.draft.repair_queue.source_ready_candidate_count, 1);
  assertEq(payload.draft.repair_queue.source_acquisition_candidate_count, 2);
  ...
  assertEq(payload.draft.repair_queue.source_acquisition_next_batch_size, 2);
  assertEq(payload.draft.repair_queue.source_acquisition_next_batch[0].canonical_slug, 'ai/draft-a');
  assertEq(payload.draft.repair_queue.source_acquisition_next_batch[1].canonical_slug, 'computer-science/api-draft');
  assert(!payload.draft.repair_queue.source_acquisition_next_batch.some(candidate => candidate.canonical_slug === 'business/brand-draft'), 'source acquisition queue should only include drafts with zero verified sources');
  ...
  assert(payload.draft.repair_queue.strict_review_next_batch.some(candidate => candidate.canonical_slug === 'ai/ai-warehouse-robotics'), 'repair queue should expose physical-safety drafts separately');
  ...
  assert(payload.draft.repair_queue.strict_review_reason_distribution.some(reason => reason.name === 'financial_or_insurance'), 'repair queue should summarize financial strict-review reasons');
  ...
  assert(payload.draft.repair_queue.selection_policy.some(item => item.includes('source acquisition')), 'repair queue should explain source acquisition routing');
